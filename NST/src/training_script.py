...
from NST.models.definitions.perceptual_loss_net import PerceptualLossNet
...
def train(training_config):
    ...
    for epoch in range(training_config['num_of_epochs']):
        for batch_id, (content_batch, _) in enumerate(train_loader):
            ...
            path_content = os.path.join('losses', training_config['style_img_name'])
            if not os.path.exists(path_content):
                os.makedirs(path_content)
            with open(os.path.join(path_content, 'content_losses_dict.pickle'), 'wb') as handle:
            ...
            # Save style loss
            style_losses_dict[epoch] = style_loss
            path_style = os.path.join('losses', training_config['style_img_name'])
            if not os.path.exists(path_style):
                os.makedirs(path_style)
            with open(os.path.join(path_style, 'style_losses_dict.pickle'), 'wb') as handle:
...
if __name__ == "__main__":
    ...
    # training related
    parser.add_argument("--style_img_name", type=str, help="style image name that will be used for training", default='madhubani.jpg')
