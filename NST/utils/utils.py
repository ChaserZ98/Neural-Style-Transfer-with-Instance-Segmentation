...
from NST.models.definitions.vgg_nets import Vgg16, Vgg16Experimental, Vgg19
...
class SimpleDataset(Dataset):
    def __init__(self, img_dir, target_width=None):
...
def save_and_maybe_display_image(inference_config, dump_img, target_shape, should_display=False):
    ...
    dump_img = cv.resize(dump_img[:, :, ::-1], dsize=target_shape)
    cv.imwrite(os.path.join(dump_dir, dump_img_name), dump_img)  # ::-1 because opencv expects BGR (and not RGB) format...
    # img = cv.imread(os.path.join(dump_dir, dump_img_name))
    # img = cv.resize(img, dsize=target_shape)
    # cv.imwrite(os.path.join(dump_dir, dump_img_name), img)
